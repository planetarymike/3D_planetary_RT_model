...
namespace O_1026_constants_detail {
  DECLARE_STATIC_ARRAY_CPP(int, n_lines, multiplet_identity, PASS_ARRAY({1, 2, 2, 3, 3, 3}))
  DECLARE_STATIC_ARRAY_CPP(int, n_lines, multiplet_index   , PASS_ARRAY({0, 1, 1, 2, 2, 2}))
  DECLARE_STATIC_ARRAY_CPP(int, n_lines, lower_level_J     , PASS_ARRAY({0, 1, 1, 2, 2, 2}))
  DECLARE_STATIC_ARRAY_CPP(int, n_lines, lower_level_index , PASS_ARRAY({0, 1, 1, 2, 2, 2}))
  DECLARE_STATIC_ARRAY_CPP(int, n_lines, upper_level_J     , PASS_ARRAY({1, 1, 2, 1, 2, 3}))
  DECLARE_STATIC_ARRAY_CPP(int, n_lines, upper_level_index , PASS_ARRAY({0, 0, 1, 0, 1, 2}))
  ...
  //  rest wavelength  
  DECLARE_STATIC_ARRAY_CPP(Real, n_lines, line_wavelength,           PASS_ARRAY({// singlet
										 102.81571 /*nm*/,
										 // doublet
  ...
  DECLARE_STATIC_ARRAY_CPP(Real, n_lines, line_wavelength_offset,   PASS_ARRAY({// singlet
										0.0 /*nm*/, 
										// doublet
										4e-5 /*nm*/, -4e-5 /*nm*/,
										// triplet
  ...
  //  Einstein A  
  DECLARE_STATIC_ARRAY_CPP(Real, n_lines, line_A,                   PASS_ARRAY({ // singlet
										4.22e7 /*s^-1*/,
										// doublet
  ...
  DECLARE_STATIC_ARRAY_CPP(Real, n_lines, line_f,                   PASS_ARRAY({// singlet
										2.01e-2, // unitless
										// doublet
										5.02e-3, 1.51e-2,
										// triplet
  ...
  DECLARE_STATIC_ARRAY_CPP(Real, n_lines, line_sigma_total,         PASS_ARRAY({line_f_coeff*line_f_array[0], // cm2 Hz
  ...
  DECLARE_STATIC_ARRAY_CPP(Real, n_lines, upper_state_decay_rate,   PASS_ARRAY({// J = 1
										//   102.6 nm branch
  ...
  DECLARE_STATIC_ARRAY_CPP(Real, n_lower, lower_state_energy,             PASS_ARRAY({/* J = 0 */ REAL(0.0281416)*erg_per_eV, // erg
										      /* J = 1 */ REAL(0.0196224)*erg_per_eV,
										      /* J = 2 */ REAL(0.0      )*erg_per_eV}))
  
  DECLARE_STATIC_ARRAY_CPP(Real, n_lower, lower_state_statistical_weight, PASS_ARRAY({ /* J = 0 */ 1, 
										       /* J = 1 */ 3,
										       /* J = 2 */ 5}))
}
