//RT_grid_gpu.h --- defines methods of RT_grid that require the CUDA compiler
#ifdef __CUDACC__
		       ...
		       const int n_subsamples = 5)
{
  ...
  for (int i_obs = index; i_obs < n_obs_vecs; i_obs += stride) {
    ...
    //   printf("Hello from block %d, thread %d: i_obs = %d, RT->grid.n_dimensions = %d\n",
    // 	     blockIdx.x, threadIdx.x, i_obs, RT->grid.n_dimensions);
    // }
